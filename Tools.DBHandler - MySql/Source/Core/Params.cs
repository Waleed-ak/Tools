...
namespace Tools.DBHandler
{
  public class Params:IParams
  {
    ...
    public Params(MySqlCommand command)
    {
      Command = command;
    }
    ...
    public static void ApplyAction(MySqlCommand command,Action<IParams> action,Action<string> log)
    {
      if(action is null)
        return;
    ...
    public static Task ApplyActionAsync(MySqlCommand command,Action<IParams> action,Action<string> log) => Task.Run(() => ApplyAction(command,action,log));
    ...
    public IParams AddSqlParam(MySqlParameter value,string param = null)
    {
    ...
    private MySqlParameter FromValue(object value,string param = null) => new(parameterName: FixName(param),value: value ?? DBNull.Value);

    private MySqlParameter ObjectToJson(object value,string param = null)
    => new()
    {
