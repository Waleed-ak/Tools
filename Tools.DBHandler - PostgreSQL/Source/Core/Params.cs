...
namespace Tools.DBHandler
{
  public class Params:IParams
  {
    ...
    public Params(NpgsqlCommand command)
    {
      Command = command;
    }
    ...
    public static void ApplyAction(NpgsqlCommand command,Action<IParams> action,Action<string> log)
    {
      if(action is null)
        return;
    ...
    public static Task ApplyActionAsync(NpgsqlCommand command,Action<IParams> action,Action<string> log) => Task.Run(() => ApplyAction(command,action,log));
    ...
    public IParams AddSqlParam(NpgsqlParameter value,string param = null)
    {
    ...
    private NpgsqlParameter FromValue(object value,string param = null) => new(parameterName: FixName(param),value: value ?? DBNull.Value);

    private NpgsqlParameter ObjectToJson(object value,string param = null)
    => new()
    {
